...
const PLTCPDetailsRenderer = ({ data, handlePreview, handleDownload }) => {
   console.log('PLTCP Data received:', data);
   ...
   return (
      <>
         <DialogDescription className="sr-only">
            Public Land Tree Cutting Permit Details
         </DialogDescription>
         ...
         <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                  ...
                  <p>{data.treeType?.length > 0 ? data.treeType.join(', ') : 'N/A'}</p>
               </div>
               <div>
                  <h3 className="font-semibold text-sm text-gray-500">Tree Status</h3>
                  <p>{data.treeStatus?.length > 0 ? data.treeStatus.join(', ') : 'N/A'}</p>
               </div>
               <div>
                  <h3 className="font-semibold text-sm text-gray-500">Land Type</h3>
                  <p>{data.landType?.length > 0 ? data.landType.join(', ') : 'N/A'}</p>
               </div>
               <div>
            ...
            {data.files && (
               <ScrollArea className="h-[300px] custom-scrollbar">
                  <div className="space-y-4 pr-4">
                     {renderFileSection(processFiles(data.files.applicationLetter), "Application Letter")}
                     {renderFileSection(processFiles(data.files.lguEndorsement), "LGU Endorsement")}
                     {renderFileSection(processFiles(data.files.homeownersResolution), "Homeowner's Resolution")}
...
export default PLTCPDetailsRenderer;
