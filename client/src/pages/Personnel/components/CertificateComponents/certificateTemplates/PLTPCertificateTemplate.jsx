...
const PLTPCertificateTemplate = forwardRef(({ certificate, application, orderOfPayment, hiddenOnPrint = [] }, ref) => {
   const certificateData = {
      ...application,
      name: application?.ownerName || application?.name,
      plantedTrees: application?.otherDetails?.plantedTrees,
      naturallyGrown: application?.otherDetails?.naturallyGrown,
      standing: application?.otherDetails?.standing,
      blownDown: application?.otherDetails?.blownDown,
      withinPrivateLand: application?.otherDetails?.withinPrivateLand,
      withinTenuredForestLand: application?.otherDetails?.withinTenuredForestLand,
   ...
   console.log('PLTP Template Data:', {
      certificateData,
      certificate,
      orderOfPayment
   });
      ...
      <div ref={ref} className="p-8 bg-white">
         <div className="text-center mb-8">
            <div className="flex justify-center items-center gap-4 mb-4">
               ...
               <div>
                  <h1 className="text-xl font-bold">PRIVATE LAND TIMBER PERMIT</h1>
         ...
         <div className="mb-6">
            <p className="mb-2">Tree Details:</p>
            <div className="grid grid-cols-2 gap-4">
               <div>
                  <p><span className="font-bold">Tree Type:</span></p>
                  <p>• Planted Trees: {certificateData?.plantedTrees ? 'Yes' : 'No'}</p>
                  <p>• Naturally Grown: {certificateData?.naturallyGrown ? 'Yes' : 'No'}</p>
                  <p><span className="font-bold">Tree Status:</span></p>
                  <p>• Standing: {certificateData?.standing ? 'Yes' : 'No'}</p>
                  <p>• Blown Down: {certificateData?.blownDown ? 'Yes' : 'No'}</p>
               </div>
               <div>
                  <p><span className="font-bold">Location:</span></p>
                  <p>• Private Land: {certificateData?.withinPrivateLand ? 'Yes' : 'No'}</p>
                  <p>• Tenured Forest Land: {certificateData?.withinTenuredForestLand ? 'Yes' : 'No'}</p>
                  <p><span className="font-bold">Other Details:</span></p>
                  <p>• Posing Danger: {certificateData?.posingDanger ? 'Yes' : 'No'}</p>
                  <p>• For Personal Use: {certificateData?.forPersonalUse ? 'Yes' : 'No'}</p>
               </div>
            </div>
            <div className="mt-4">
...
PLTPCertificateTemplate.displayName = 'PLTPCertificateTemplate';

export default PLTPCertificateTemplate;
